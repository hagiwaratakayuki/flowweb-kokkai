...
'''
import re
tabPattern = re.compile(u"^\s+", re.U)
whiteSpace = re.compile(u"\s", re.U)
symbolOnly = re.compile(u"^[\w\s]+$", re.U)
def countTab(line):
...
class Service(object):
    ...
    def isActivate(self,line):
        match = re.search(u"^(\s*)出席小?委員",line,re.U)
        if match:
    ...
    def execute(self,line,log):
        ...
        if depth > self.depth:
            ...
            vector = -1
        self.depth = depth
        if not line.count(u"君"):
            self.pending = line
            return
        if self.pending:
            if vector == 0:
                
            for split in line.split(u"君"):
                split = tabPattern.sub(split,u"")

...
class Client(object):
    ...
    def setQueryParamByDict(self, params: dict, add=False):
        for key, value in params.items():
            self.setQueryParam(key, value, add)
    ...
    def __sendRequest(self, endpoint, method, queryparam, headerparam, pathparam, parser_obj):
        ...
        if method == GET:
            url = self.getURL(endpoint, pathparam,
                              queryparam, headerparam, method)
        else:
            """
            for key in queryparam:
                param = queryparam[key]
                if isinstance(param, unicode):
                    queryparam[key] = param.encode("utf-8")
            """
            url = self.getURL(endpoint, pathparam, {}, headerparam, method)
            if hasattr(self, '_auth'):

                self._auth.sign(url, queryparam, headerparam, method)

            queryparam = urllib.parse.urlencode(queryparam)
    ...
    アクセス時の下請け兼キャッシュキー生成
        ...
        if method == GET and hasattr(self, '_auth') and (not self._auth is None):
        ...
        if queryparam:
            ...
            for key, value in queryparam.iteritems():
                ...
                qs = key + "=" + urllib.parse.quote_plus(value)
                querys.append(qs)

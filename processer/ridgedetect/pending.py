...
class Taged(RidgeDitect):
    ...
    def _mapper(self, cluster_members):
        ...
        new_clusters = {}
        tag_member_map = defaultdict(set)
        tags_2_members = defaultdict(set)
        ...
        for cluster_member in cluster_members:
            ...
            for tag in tags:
                tag_member_map[tag].add(cluster_member)
                tags_2_members[frozenset([tag])].add(cluster_member)
            if len(tags) > 1:
        ...
        for tag in tag_paires.keys():
            ...
            target_paths = deque([(frozenset({tag}), tag,)])
            is_path_link_exist = True
            while is_path_link_exist == True:
                next_target_path = deque()
                is_path_link_exist = False
                for target_tags, tail in target_paths:

                    path_members = tags_2_members[target_tags]

                    for link in tag_paires.get(tail, EMPTY_SET):
                        if link in target_tags:
                            continue
                        next_tags = target_tags | frozenset({link})

                        if next_tags in checked:
                            continue
                        checked[next_tags] = True
                        members_set = tag_member_map[link] & path_members

                        tags_2_members[next_tags] = members_set
                        is_path_link_exist = True
                        next_target_path.append((next_tags, link,))
                target_tags = next_target_path
